...
def test_model():
    config_path = '../../configs/my_configs/cluster2one_transformer_config.py'
    config = mmcv.Config.fromfile(config_path)
    # 需修改
    # config.data_root = '/Users/kyanchen/Code/DetFramework/data/nms_net/'
    config.data_root = '../../data/nms_net/'
    config.data.test.ann_file = config.data_root + 'instances_val2017.json'
    config.data.test.proposal_file = config.data_root + 'ori_val_wo_nms.bbox.json'

    dataset = test_dataset_init(config)

    eval_kwargs = config.get('evaluation', {}).copy()
    eval_kwargs['patterns'] = {'pattern':'coco_json', 'nms_cfg':dict(type='nms', iou_threshold=0.6), 'max_per_img':100}
    eval_kwargs.pop('interval')
    metric = dataset.evaluate(results=None, **eval_kwargs)
    print(metric)
