...
class FasterRCNN_CLIP_Prompter_Region(BaseModule):
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(FasterRCNN_CLIP_Prompter_Region, self).__init__(init_cfg)
        if pretrained:
        ...
        if kd_model:
            ...
            # self.kd_img_align = nn.Linear(1024, 1024) # R50
    ...
    def forward_train(self,
        ...
        if self.box_reg == 'coco+vaw':
            ...
            if torch.any(coco_seen_mask):  # 存在coco seen 目标
                ...
                # gt_labels_rpn = [torch.ones(len(boxes_rpn[idx]), dtype=torch.long).to(img.device) for idx in range(len(boxes_rpn))]
                # roi_losses = self.roi_head.forward_train(img_rpn, img_metas_rpn, proposal_list,
                #                                          boxes_rpn, gt_labels_rpn,
                #                                          gt_bboxes_ignore=None, gt_masks=None,
                #                                          **kwargs)
                # losses.update(roi_losses)
            else:
                # roi_losses = {
                #     'loss_bbox': torch.tensor(0.).to(img.device),
