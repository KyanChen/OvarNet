...
class OvarNetAttributeDataset(Dataset):
    ...
    def __init__(self,
        ...
        super(OvarNetAttributeDataset, self).__init__()
    ...
    def _filter_imgs(self, min_wh_size=32, min_box_wh_size=4, loose_format=True):
        valid_img_ids = []
    ...
    def get_img_instances(self, idx):
            ...
            if data_set == 'vaw' or data_set == 'ovadgen':
                ...
                for att in negative_attributes:
                    if att in self.att_seen_unseen['seen']:
                        ...
                        if att_id is not None:
                            labels[att_id] = 0
                if np.all(labels[:len(self.att2id)] == 2):
                    instance_source = 2  # only carry on kl loss, ignore reg and cls supervision
            if data_set == 'coco':
                instance_source = 0
                category = instance['name']
                if category in self.category_seen_unseen['seen']:
                    ...
                    instance_source = 3  # only carry on kl loss, ignore reg and cls supervision
            attr_label_list.append(labels)
            instance_sources.append(instance_source)

        gt_bboxes = np.array(bbox_list, dtype=np.float32)
        instance_sources = np.array(instance_sources, dtype=np.int)
        gt_labels = np.stack(attr_label_list, axis=0).astype(np.int)

        results['gt_bboxes'] = gt_bboxes
        results['bbox_fields'] = ['gt_bboxes']
        results['gt_labels'] = gt_labels
        results['instance_sources'] = instance_sources
        ...
        try:
            ...
            results['instance_sources'] = DataContainer(results['instance_sources'], stack=False)
