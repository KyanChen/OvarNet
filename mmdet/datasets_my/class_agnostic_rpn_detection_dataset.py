...
class ClsAgnosticRPNDataset(Dataset):
    ...
    def __init__(self,
        ...
        super(ClsAgnosticRPNDataset, self).__init__()
        ...
        if 'category_file' in attribute_index_file.keys():
            ...
            if att_group in ['common1', 'common2', 'common', 'rare']:
                ...
                self.category_seen_unseen['seen'] = list(category2id['common'].keys())
                self.category_seen_unseen['unseen'] = list(category2id['rare'].keys())
            else:
                raise NameError
        ...
        self.id2images = {}  # {'img_id': img_info}
        self.id2instances = {}  # {'img_id': [instance_0, instance_1]}
        ...
        for x in self.img_ids:
            dataset_name = x.split('_')[0]
            imgids_per_dataset[dataset_name] = imgids_per_dataset.get(dataset_name, []) + [x]

        rank, world_size = get_dist_info()
        if rank == 0:
            for k, v in imgids_per_dataset.items():
                print('dataset: ', k, ' len - ', len(v))
        if dataset_balance and not test_mode:
            balance_frac = round(len(imgids_per_dataset['coco']) / len(imgids_per_dataset['vaw']))
            self.img_ids = imgids_per_dataset['coco'] + balance_frac * imgids_per_dataset['vaw']
            if rank == 0:
                print('balance dataset fractor = ', balance_frac)
                data_len = {}
                for x in self.img_ids:
                    dataset_name = x.split('_')[0]
                    data_len[dataset_name] = data_len.get(dataset_name, 0) + 1
                for k, v in data_len.items():
                    print('balanced data len: ', k, ' - ', v)
    ...
    def read_data_coco(self, pattern):
        ...
        id2category = {x['id']: x['name'] for x in json_data['categories']}
        ...
        for data in json_data['annotations']:
            ...
            data['name'] = id2category[data['category_id']]
            ...
            if data_set == 'coco':
                ...
                if category in self.category_seen_unseen['seen']:
                    ...
                    if category_id is not None:
                        ...
                        attr_label_list.append(0)
            elif data_set == 'vaw':
                data_set_type = 1
                bbox_list.append([x, y, x + w, y + h])
                attr_label_list.append(0)
        gt_bboxes = np.array(bbox_list, dtype=np.float32)
        gt_labels = np.array(attr_label_list).astype(np.int)
    ...
    def get_labels(self):
        ...
        for idx in range(len(self)):
            ...
            gt_labels = np.array(attr_label_list, dtype=np.int).reshape(-1, 1)
    ...
    def evaluate_box_free(self, results, nms_cfg):
        ...
        for pred in predictions:
            ...
            # pred_scores = (pred_scores * pred_proposal_scores[:, None]) ** 0.5

            proposal_score_thr = nms_cfg.pop('proposal_score_thr', 0.1)
            ...
            score_thr = nms_cfg.pop('score_thr', 0.1)
    ...
    def evaluate(self,
        ...
        if self.test_content == 'box_free':
            # save_data = {}
            #
            # for k, v in zip(self.img_ids, results):
            #     save_data[k] = v.cpu()
            # # for k, v in zip(self.img_ids[-2000:], results[-2000:]):
            # #     save_data[k] = v.cpu().numpy()
            # torch.save(save_data, 'pred_box_free.pth')
            # # mmcv.dump(save_data, 'pred_box_free.pkl')
            # import pdb
            # pdb.set_trace()
