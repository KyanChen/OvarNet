...
class LSARPNAttributeDataset(Dataset):
    ...
    def __init__(self,
        ...
        super(LSARPNAttributeDataset, self).__init__()
        ...
        # json.dump(self.img_ids, open('img_ids.json', 'w'))
        # self.redis_helper = RedisHelper()
        # for img_id in self.img_ids:
        #     v = {f'imginfo_{img_id}': id2images_lsa[img_id],
        #          f'instanceinfo_{img_id}': id2instances_lsa[img_id]
        #          }
        #     self.redis_helper.set_values(img_id, v)
    ...
    def _filter_imgs(self, min_wh_size=32, min_box_wh_size=4):
        ...
        for img_id, img_info in self.id2images.items():
            ...
            self.id2instances[img_id] = instances
            if len(instances) == 0:
                continue
    ...
    def get_img_instances(self, idx):
        ...
        for instance in instances[:15]:
            ...
            category_phases.append(instance.get('object', 'object'))
        ...
        try:
            ...
            results['phases'] = DataContainer(category_phases, stack=False, cpu_only=True)
            if self.kd_pipeline:
    ...
    def get_test_box_given(self, idx):
        ...
        for instance in instances:
            ...
            if x1 == -1 and y1 == -1 and x2 == -1 and y2 == -1:  # 全图，有"instance_id" "ground": "none", COCO
                ...
                l, t, r, b = x1 * img_info['width'], y1 * img_info['height'], x2 * img_info['width'], y2 * img_info[
                    'height']
            elif data_set == 'vg' and x2 > 1 and y2 > 1:  # 有"instance_id" "ground": "box" VG
                l, t, r, b = x1, y1, x1 + x2, y1 + y2
            elif data_set == 'flickr' and x2 > 1 and y2 > 1:  # 有"instance_id" "ground": "box","none" flickr
    ...
    def get_labels(self):
        ...
        for idx in range(len(self)):
            ...
            for instance in instances:
                ...
                r = np.clip(r, 0, img_info['width'])
                b = np.clip(b, 0, img_info['height'])
                bbox_list.append([data_set_type, l, t, r, b])
    ...
    def evaluate_box_given(self, results):
        ...
        n = torch.from_numpy(data_set_type)
