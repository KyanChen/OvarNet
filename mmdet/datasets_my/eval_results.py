...
class InferenceRPNAttributeDataset(Dataset):
    ...
    def evaluate_box_free(self, results, nms_cfg):
        ...
        # gt_bboxes = [gt[:, 0:4] for gt in gt_labels]
        ...
        gt_bboxes = [gt for gt in gt_labels]
        predictions = [x.cpu().numpy() for idx, x in enumerate(results)]

        prediction_attributes = []
        gt_attributes = []
        # predictions List[Tensor] N, Nx(4+1+620)
        # gts List[Tensor] N, Nx(4+620)
        for prediction, gt in zip(predictions, gt_bboxes):
            IoUs = bbox_overlaps(prediction[:, :4], gt[:, :4])
            inds = np.argmax(IoUs, axis=0)
            pred_att = prediction[inds][:, 4:4+len(self.att2id)]
            gt_att = gt[:, 4:4 + len(self.att2id)]
        ...
        pred_att_logits = torch.from_numpy(prediction_attributes).float().numpy()  # Nx620
